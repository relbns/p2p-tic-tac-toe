...
export const useGame = () => {
  ...
  const [myTurn, setMyTurn] = useState(true); // Start with true for local games
    ...
    if (gameEnd) {
      ...
      if (gameEnd.type === 'win') {
        setGameResult({
          type: 'win',
          winner: gameEnd.winner,
          isPlayerWin: false // Not applicable for local games
        });
      } else {
        setGameResult({ type: 'tie' });
      }
    } else {
      // Switch turns for local game
      setMyTurn(!myTurn);
    }
  ...
  const resetGame = useCallback(() => {
    ...
    setMyTurn(true); // Always start with player 1 (X) for local games
