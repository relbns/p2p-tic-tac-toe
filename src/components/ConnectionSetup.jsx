...
const ConnectionSetup = ({
  ...
  <div className="space-y-6">
    ...
    <div
      className={`p-4 rounded-xl min-h-[60px] flex items-center justify-center border
      ${
        status.type === 'success'
    ...
    {selectedMethod && !isHosting && !isJoining && (
      <div className="flex gap-3">
        <button
          onClick={onHost}
          className="flex-1 p-3 rounded-xl bg-white/20 hover:bg-white/30 font-semibold transition-all backdrop-blur border border-white/30"
        >
          Host Game
        </button>
        <button
    ...
    {isHosting && (
      <div className="text-center space-y-4">
        ...
        <div className="text-4xl font-bold font-mono tracking-widest p-4 bg-black/20 rounded-xl border-2 border-dashed border-white/50 cursor-pointer hover:bg-black/30 transition-all">
          {gameCode}
        </div>
    ...
    {isJoining && (
      <div className="space-y-4">
        <input
          ...
          className="w-full p-4 text-center text-2xl font-mono tracking-widest bg-white/10 border border-white/30 rounded-xl text-white placeholder-white/70 backdrop-blur"
          maxLength={4}
        />
        <button
          onClick={onConnect}
          className="w-full p-3 rounded-xl bg-white/20 hover:bg-white/30 font-semibold transition-all backdrop-blur border border-white/30"
        >
          Connect
        </button>
      </div>
