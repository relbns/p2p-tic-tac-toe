...
import ConnectionTroubleshooting from './ConnectionTroubleshooting';
...
const ConnectionSetup = ({
  ...
  onShareCode,
  onCancel,
  onRetryConnection, // New prop for retry functionality
}) => {
  // Check if we should show troubleshooting
  const shouldShowTroubleshooting = 
    selectedMethod === 'webrtc' && 
    status.type === 'error' && 
    (status.message.includes('Connection timeout') || 
     status.message.includes('Connection failed') ||
     status.message.includes('WebRTC error') ||
     status.message.includes('Network error') ||
     status.message.includes('Failed to connect'));
  ...
  const handleSwitchToHotspot = () => {
    onMethodSelect('hotspot');
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Show troubleshooting component when WebRTC fails */}
      {shouldShowTroubleshooting && (
        <ConnectionTroubleshooting
          onRetry={handleRetry}
          onSwitchMethod={handleSwitchToHotspot}
        />
      )}
      ...
      {isJoining && (
        <div className="space-y-4">
          ...
          <button
            ...
            className="w-full p-3 rounded-xl bg-white/20 hover:bg-white/30 font-semibold transition-all backdrop-blur border border-white/30 text-white select-none disabled:opacity-50 disabled:cursor-not-allowed"
          >
